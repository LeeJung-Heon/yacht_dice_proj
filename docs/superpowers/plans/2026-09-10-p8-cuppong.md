...
- [x] **Step 2: 실패 확인** — `swift test --package-path Packages/GameCore 2>&1 | tail -3` → `CupPong` 없음.
...
- [x] **Step 4: 통과 확인** — `swift test --package-path Packages/GameCore` → 기존 7 + 새 8 = 15 통과. `완주`가 특정 인덱스에서 실패하면 그 힘의 착지(`landing`)를 출력해 Global Constraints의 수식과 대조한다 — 수식이 맞으면 테스트의 힘 값이 틀린 것이므로 해당 컵 중심에 맞게 `dx = cx * 1500 / L`을 다시 계산한다.

- [x] **Step 5: 커밋** — `git add Packages/GameCore && git commit -m "feat(core): 컵퐁 규칙을 GameCore에 둔다"`
...
- [x] **Step 2: 실패 확인** — `startCupPongLocal`·`playingCupPong` 없음.
...
- [x] **Step 4: 통과 확인** — 단위 전체(새 5개 포함)와 `-only-testing:YachtDiceUITests/MenuUITests`.

- [x] **Step 5: 커밋** — `git add -A && git commit -m "feat(cuppong): 허브·메뉴·컨테이너에 컵퐁을 잇는다"`
...
- [x] **Step 2: 실패 확인** — `CupPongGeometry` 없음.
...
- [x] **Step 5: 통과 확인** — `-only-testing:YachtDiceTests/CupPongGeometryTests` 3개. 앱 타깃도 빌드된다(`CupPongTableView`는 아직 아무도 쓰지 않는다).

- [x] **Step 6: 커밋** — `git add -A && git commit -m "feat(cuppong): 원근 테이블 뷰와 스와이프 → 힘 변환"`
...
- [x] **Step 2: 실패 확인** — `SoundSynth.pong` 없음, UI 테스트는 자리 표시자에 `cuppong.table`이 없어 실패.
...
- [x] **Step 4: 통과 확인** — `-only-testing:YachtDiceTests/FeedbackTests`, `-only-testing:YachtDiceUITests/CupPongUITests`, 그리고 UI 전체 한 번. 스와이프가 테이블 위에서 인식되지 않으면 `DragGesture`를 `CupPongTableView`가 아니라 `GeometryReader` 전체(`.contentShape(Rectangle())`)에 건다.

- [x] **Step 5: 커밋** — `git add -A && git commit -m "feat(cuppong): 던지기·재생·결과 화면"`
...
- [x] **Step 2: 통합 실행** — 한 번. 익명 가입 한도(시간당 30회)에 걸리면 한 시간 뒤 다시 돈다.

- [x] **Step 3: 문서** — README의 게임 허브 절에 컵퐁 한 문단(규칙, 힘만 보내는 결정적 착지, 화면 시점)을 더하고, 스펙 상태를 `구현 완료 (날짜)`로 바꾼다. 계획 체크박스를 켠다(이 Task의 Step 5는 컨트롤러가 머지 뒤 한다고 적는다).

- [x] **Step 4: 전체 검증** — GameCore 패키지, 단위 전체, UI 전체, 통합. 모두 통과.

- [x] **Step 5: 빌드 번호** — `project.yml`의 `CURRENT_PROJECT_VERSION`을 6으로 올리고 `xcodegen generate`. TestFlight 업로드·그룹 연결·심사 제출은 머지 뒤 컨트롤러가 한다.

- [x] **Step 6: 커밋** — `git add -A && git commit -m "docs: P8 완료 반영과 빌드 6"`
