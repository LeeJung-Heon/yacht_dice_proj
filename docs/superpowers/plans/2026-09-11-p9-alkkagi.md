...
- [x] **Step 2: 실패 확인** — `swift test --package-path Packages/GameCore 2>&1 | tail -3` → `Alkkagi` 없음.
...
- [x] **Step 2: 실패 확인** — `startAlkkagiLocal`·`playingAlkkagi` 없음.

- [x] **Step 3: 구현** — `App/AppContainer.swift`의 컵퐁 갈래를 하나씩 따라 알까기 갈래를 더한다(`Status.playingAlkkagi`, `currentMatchID`, `currentGame`, `openOnlineMatchID`, `resumeSavedGame`의 `case .alkkagi: if !launchAlkkagi(record, transport: nil) { discardSavedGame() }`, `stopCurrentGame`, `startAlkkagiLocal(names:)`, `launchAlkkagi(_:transport:) -> Bool`(참가자 수·`moveLog` 디코드 가드, `onFinished`, 저장 클로저, `startListening`, `stopCurrentGame()` 뒤 `status = .playingAlkkagi`), `openSupabaseMatch`의 `.alkkagi` 갈래). `App/Games/GameCatalog.swift`: `var isAvailable: Bool { true }`. `App/YachtDiceApp.swift`: `.menu(.alkkagi) → AlkkagiMenu`, `.playingAlkkagi → AlkkagiScreen(match:onReturn:)`. `App/Games/Alkkagi/AlkkagiMenu.swift`: `App/Games/CupPong/CupPongMenu.swift`를 복제해 제목 "알까기"·부제 "돌을 튕겨 밀어낸다", 이름 기본값 `["흑", "백"]`, 식별자 `menu.alkkagi.*`, 이어하기 조건 `saved.game == "alkkagi"`, 온라인 `OnlineMenu(container: container, game: .alkkagi)`, 시작 `container.startAlkkagiLocal(names:)`. `App/Games/Alkkagi/AlkkagiScreen.swift` 자리 표시자는 컵퐁 Task 2의 것과 같은 모양(제목 "알까기", `header.menu`).

- [x] **Step 4: 통과 확인** — 단위 전체와 `-only-testing:YachtDiceUITests/MenuUITests`.

- [x] **Step 5: 커밋** — `git add -A && git commit -m "feat(alkkagi): 허브·메뉴·컨테이너에 알까기를 잇는다"`
...
- [x] **Step 2: 실패 확인** — `AlkkagiGeometry` 없음.
...
- [x] **Step 5: 통과 확인** — `-only-testing:YachtDiceTests/AlkkagiGeometryTests` 3개, 앱 타깃 빌드.

- [x] **Step 6: 커밋** — `git add -A && git commit -m "feat(alkkagi): 판 기하와 판 뷰"`
...
- [x] **Step 2: 실패 확인** — `SoundSynth.clack` 없음; UI 테스트는 자리 표시자에 `alkkagi.board`가 없어 실패.
...
```
주의: `onRemoteMove`는 로그에 더해지기 전에 불리므로 `match.state`는 수 전 상태이고 `simulate`가 `apply`와 같은 결과를 낸다. 재생이 끝난 뒤 `playing = nil`이 되면 그 직후 `append`가 상태를 바꾸어 같은 배치가 보인다(내 튕김은 수를 먼저 보내고 첫 프레임을 미리 그려 두므로 재생과 `apply` 결과가 어긋나 보이지 않는다). 뒤집힌 판에서 `AlkkagiGeometry.point`가 좌표를 되돌리므로 `flick`의 방향은 격자 기준으로 맞다. 600스텝 상한은 닿지 않는 여유라, 최대 속력 30000이 마찰 100/스텝에 300스텝 안에 멎고 반발 9/10이 속력을 키우지 못해 시작 배치의 최악이 163스텝이니 재생은 길어야 1.5초쯤에 끝난다.

- [x] **Step 4: 통과 확인** — `-only-testing:YachtDiceTests/FeedbackTests`, `-only-testing:YachtDiceUITests/AlkkagiUITests`, UI 전체 한 번. 드래그가 돌을 잡지 못하면 UI 테스트의 정규화 좌표를 `AlkkagiGeometry.project(Point(x: 6000, y: 2000))`의 값(`(0.5, 1 - 3/14 ≈ 0.786)`)과 판의 실제 프레임(정사각형 `aspectRatio`)으로 대조한다.

- [x] **Step 5: 커밋** — `git add -A && git commit -m "feat(alkkagi): 새총 조준·재생·결과 화면"`
...
- [x] **Step 1: 통합 테스트** — `SupabaseE2ETests.swift`에 컵퐁 테스트(`컵퐁_한_판`)와 같은 골격으로 `알까기_한_판`을 더한다: `createRoom(name: "A", game: "alkkagi", player: nil)`, `OnlineMatch<Alkkagi>` 둘, 구독 대기, 수 목록은 `AlkkagiMatchTests.winningMoves()`와 같은 아홉 수를 좌석 번갈아(짝수 인덱스는 호스트, 홀수는 게스트) 두며 상대의 `log.moves.count`가 따라올 때까지 최대 10초 기다린다. 끝나면 `a.outcome == .win(seat: 0)`, 서버 행 `status == "finished"`, `winnerSeat == 0`, `game == "alkkagi"`.

- [x] **Step 2: 통합 실행** — 한 번. 익명 가입 한도에 걸리면 한 시간 뒤.

- [x] **Step 3: 문서** — README 게임 허브 절에 알까기 문단(규칙, 힘만 보내는 정수 시뮬레이션, 새총 조준, 뒤집힌 판), 허브 타일 넷 모두 활성으로 표기, 스펙 상태 `구현 완료 (날짜)`(실기기 손맛 확인 남음), 계획 체크박스.

- [x] **Step 4: 전체 검증** — GameCore 패키지, 단위 전체, UI 전체, 통합.

- [x] **Step 5: 빌드 번호** — `CURRENT_PROJECT_VERSION: 7`, `xcodegen generate`. 업로드·그룹 연결·심사 제출은 머지 뒤 컨트롤러가 한다.

- [x] **Step 6: 커밋** — `git add -A && git commit -m "docs: P9 완료 반영과 빌드 7"`
