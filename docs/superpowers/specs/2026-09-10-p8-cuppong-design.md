...
3. 상대 수는 `onRemoteMove`로 같은 애니메이션을 재생한다. 화면은 던지는 사람의 시점이라 먼 쪽 삼각형은 상대 차례에 내 컵이 되며 헤더가 "상대가 내 컵을 노린다"고 알린다.
...
끌기 시작점에서 끝점까지의 벡터 `(vx, vy)`(위가 양수)와 걸린 시간 `t`로 `power = clamp(길이 / 화면 높이 * 1400 * (1 + 0.3 * min(1, 속도 계수)), 0, 1000)`, `dx = clamp(vx / max(|vy|, 1) * 1000, -1000, 1000)`을 만든다. 위로 끌지 않으면(`vy <= 0`) 던지지 않는다. 값은 정수로 반올림하여 그대로 수가 된다.
