...
## 2026-09-22 개정: 넓은 판과 3판 2선승

이 절이 아래 초기 설계의 판 크기·승패·배치 순서를 대체한다.

- 실제 판은 **17줄, 좌표 `0…16000`**이다. 돌 크기와 줄 간격은 유지한다. 기본 배치는 x=`6000…10000`, 흑 y=`3000`, 백 y=`13000`이다. 진영은 흑 `0…7000`, 백 `9000…16000`이다. 화면에서는 돌 반지름 400만 여백으로 남겨 교차점 영역이 판 너비의 95% 이상을 차지하고, 판 바깥 가로 여백은 4pt다.
- **두 라운드를 먼저 이기면 경기 승리**다. 각 라운드는 돌 다섯 개씩 새로 배치한다. 선공은 흑→백→흑이며 배치도 선공부터 한다. 라운드 종료는 `roundOutcome`, 누적 승수는 `roundWins`, 전체 경기 종료만 `outcome`에 기록한다. 2:0이면 3라운드를 열지 않는다.
- 맵은 **클래식 → 깎인 모서리 → 중앙 범퍼**다. 두 번째 맵의 네 모서리는 실제 낙하 영역이다. 세 번째 맵의 중앙 반지름 1600 범퍼는 고정 충돌체이며 돌이 반발계수 0.9로 튕긴다. 배치 검증도 잘린 영역과 범퍼를 피한다. 모든 충돌·낙하는 기존 정수 시뮬레이션으로 재현한다.
- 라운드 종료 화면에서 **다음 라운드**를 누르면 `.nextRound`가 수 로그와 온라인 전송에 기록된다. 다음 라운드 선공만 이 수를 둘 수 있다. 무승부는 승수를 바꾸지 않고 같은 라운드 번호·맵·선공으로 다시 배치한다.
- 화면에는 라운드 번호·맵 이름·각 좌석 승수·남은 돌을 표시한다. 범퍼 충돌도 돌 충돌과 같은 소리·햅틱을 낸다. 다음 라운드나 다른 재생이 시작되면 이전 재생 Task는 프레임·효과음을 더 이상 쓰지 않는다.
- `State` 저장에는 `roundNumber`, `roundWins`, `roundOutcome`을 추가했다. 구형 상태를 읽을 때는 첫 라운드 기본값을 쓰며, 게임 수 로그의 규칙 버전 검증은 공통 `MoveLog` 정책을 따른다.
- 검증: 새 영역 배치, 첫 승 후 경기 유지, 2승 종료, 선공 교대, 1:1 뒤 세 번째 맵, 무승부 재경기, 모서리 낙하, 범퍼 반사, 저장 및 두 라운드 로그 재생, 메모리 온라인 수 전파를 검사한다.
...
| 재현 | 1/120초 고정 간격 정수 시뮬레이션(사칙연산과 정수 제곱근만). 최대 600스텝. 4스텝마다 프레임을 기록해 재생 목록을 돌려주고, 화면은 프레임 사이를 시간에 따라 직선 보간해 화면 주사율로 그린다 |
...
| `App/Games/Alkkagi/AlkkagiScreen.swift` | 한 줄 헤더(`header.menu`, `header.status` = "내 돌 N · 상대 돌 M"이고 배치 중에는 "배치 중", `header.turn` = "<이름> 차례"이고 배치 중에는 "<이름> 배치"), 배치 단계(초안 `draft`를 끌어 옮기고 놓을 때 `snap`·`placementIsValid`로 거르며 "배치 완료" `alkkagi.placeDone`가 `.setup`을 둔다), 명패 둘(`players.seat.<i>`, `players.presence.<i>`), 드래그(내 돌 위에서 시작한 것만) → 놓으면 `match.play(Flick)`, 프레임 재생(`TimelineView`가 시작 시각부터 흐른 스텝으로 프레임 사이를 보간해 화면 주사율로 그리고, 각 프레임이 놓인 벽시계 시각에 충돌 프레임의 `SoundSynth.clack`·햅틱과 낙하의 `drop`을 낸다), 상대 수 재생(`onRemoteMove`가 `simulate`로 프레임을 만들어 같은 재생), 차례 배너·토스트·연결 띠·결과 카드(`alkkagi.result`, `alkkagi.back`), `scenePhase` resync |
...
3. 화면은 `state.lastSimulation.frames`를 시작 시각부터 흐른 스텝(초당 120)으로 프레임 사이를 보간해 화면 주사율로 그리며, `collision` 이벤트 프레임에 `clack`, `dropped`에 `drop`을 그 프레임의 벽시계 시각에 낸다. 다음 프레임에서 사라진 돌은 그 칸이 끝날 때까지 있던 자리에 둔다. 재생 중 입력은 잠긴다.
...
| 기하 단위 | `project`/`point` 왕복과 뒤집기 대칭, 바깥 여백이 반 칸(`unit = 폭 / 13000`, `project(0, 0)`이 가장자리에서 반 칸 안), `snap`이 가장 가까운 교차점으로 붙이고 판 안에 묶는다, 끌기 → `Flick` 범위와 방향 반전, 짧은 끌기 nil |
| `OnlineMatch<Alkkagi>` | 메모리 쌍으로 수 전파·재생·완주 |
| 서버 통합 | `game = 'alkkagi'` 방에서 정해진 수 목록으로 끝까지 가 `winner_seat` |
| UI | 허브 → 알까기 로컬 2인 → "배치 완료"를 흑·백으로 두 번(둘째는 단추가 다시 열리기를 기다린다) → 내 돌을 당겨 놓기 → `header.turn`이 바뀐다 |
