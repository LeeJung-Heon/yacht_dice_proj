...
struct OmokBoardTests {
    ...
    @Test("정사각형이 아닌 자리에서는 짧은 변이 판의 크기다",
          arguments: [CGSize(width: 380, height: 520), CGSize(width: 520, height: 380)])
    func 짧은_변을_따른다(size: CGSize) {
        #expect(OmokGeometry.cell(size) == 20)
        #expect(OmokGeometry.point(of: Omok.Move(x: 18, y: 18), in: size) == CGPoint(x: 370, y: 370))
        for (x, y) in [(0, 0), (9, 9)] {
            let p = OmokGeometry.point(of: Omok.Move(x: x, y: y), in: size)
            #expect(OmokGeometry.move(at: p, in: size) == Omok.Move(x: x, y: y))
        }
        // 380pt 정사각형 밖은 긴 변 쪽으로 남는 자리라도 판이 아니다
        #expect(OmokGeometry.move(at: CGPoint(x: 150, y: 400), in: size) == nil)
        #expect(OmokGeometry.move(at: CGPoint(x: 400, y: 150), in: size) == nil)
    }
}
