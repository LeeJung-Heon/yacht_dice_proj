...
오목은 실제 플레이 영역을 **19×19 교차점**으로 넓혔다. 흑부터 번갈아 두며 가로·세로·대각선으로 다섯 개 이상 이으면 승리하고 금수는 없다. 화면도 좌우 여백을 줄여 판을 크게 표시하며, 나뭇결·화점 아홉 개·돌의 광택을 그린다. 교차점을 선택하면 좌표를 확인하고 “놓기”로 확정한다.

컵퐁은 상대의 컵 열 개를 먼저 모두 비우면 이기는 게임이다. 상대 컵에 넣으면 같은 사람이 다시 던지고, 빗나가면 차례가 넘어간다. `CupPongPhysics`는 `Shot { dx, power }`를 1/240초 정수 시뮬레이션으로 재생하며 중력·공기 저항·테이블 바운스·컵 테두리와 벽의 충돌을 계산한다. 화면은 같은 시뮬레이션의 공 위치와 충돌 사건을 사용하므로 튕김과 판정이 일치하고 온라인 상대도 같은 결과를 본다. 던지기 전 예상 궤적과 착지 표시는 없다.

컵퐁의 3D 장면은 속이 열린 테이퍼 컵, 말린 테두리와 표면 굴곡, 액체 표면, 코팅된 테이블을 코드로 만든다. 재질별 반사와 거칠기, 조명·접지 그림자로 플라스틱·탁구공·테이블의 질감을 구분한다. 던지는 사람의 시점에서 먼 쪽 컵이 목표이며, 넣거나 테두리·테이블에 닿으면 해당 사건에 맞춰 소리와 진동을 낸다.

컵퐁 메뉴의 **나만의 컵**에서 컵 1~10을 골라 사진 보관함의 이미지를 각각 넣거나 지울 수 있다. 사진은 회전을 보정하고 가운데를 512×512로 잘라 컵 앞 곡면에 붙이며 3D 미리보기에서 확인한다. 기기에 저장되어 재실행 뒤에도 유지되고 온라인에서는 `cuppong_designs`를 통해 상대에게 공유된다. 사진 행은 본인과 실제 컵퐁 대전 상대만 읽고 본인만 고칠 수 있으며 게임 로그에는 사진을 넣지 않는다. 온라인 화면은 좌석별 소유자의 디자인을 보여 주고 10초마다 버전을 확인해 변경된 사진만 받는다. 사진 업로드가 실패하면 로컬 저장을 유지하고 편집 화면에 재시도를 표시한다.

알까기는 **17줄 판에서 3판 2선승제**로 진행한다. 1라운드는 클래식, 2라운드는 모서리가 잘린 판, 3라운드는 중앙 범퍼가 있는 판이다. 잘린 모서리는 실제 낙하 영역이고 범퍼는 돌을 튕겨 낸다. 각 라운드에서 상대 돌 다섯 개를 모두 밀어내면 1승을 얻으며, 두 라운드를 먼저 이기면 경기가 끝난다. 양쪽 마지막 돌이 함께 떨어지면 점수를 더하지 않고 같은 맵에서 해당 라운드를 다시 시작한다.

라운드마다 자기 진영의 교차점에 돌 다섯 개를 배치하고, 선공은 흑·백·흑 순서로 바뀐다. 돌이나 범퍼와 겹치거나 잘린 모서리에 놓는 배치는 거부한다. 다음 라운드 선공인 플레이어가 “다음 라운드”를 누르면 양쪽 화면이 새 배치 단계로 넘어간다. 판은 화면 좌우 여백을 줄여 크게 표시하며 점수·현재 라운드·맵 이름·남은 돌 수를 함께 보여 준다.

알까기의 `Move`는 배치·튕김·다음 라운드 진행을 기록한다. 이동·마찰·돌끼리의 충돌·범퍼 반사·낙하는 1/120초 정수 시뮬레이션으로 계산하고, 화면은 프레임 사이를 보간해 화면 주사율에 맞춰 부드럽게 재생한다. 내 돌을 당겼다 놓는 새총식 입력을 쓴다. 조준선은 초기 이동만 최대 약 2.5칸까지 짧게 표시하고 끝으로 갈수록 흐려진다. 돌 충돌·범퍼·낙하 뒤의 경로와 최종 정지점은 표시하지 않으며, 온라인 게스트의 화면은 내 돌이 아래에 오도록 판을 180도 돌린다.
...
**규칙 2 배포:** `supabase/migrations/20260922_game_rules_version.sql`은 2026-09-22 운영 DB에 적용했다. 새 `join_match`는 게임과 규칙 버전을 확인한 뒤 자리를 배정하므로 구버전 앱과 새 규칙의 방이 섞이지 않는다. 오목·컵퐁·알까기의 기존 규칙 1 기록은 새 규칙으로 이어할 수 없으며 새 판을 시작해야 한다. 요트 기록은 유지된다.

2026-09-22 사용자가 모든 디자인과 규칙을 확정했다. 규칙 2 서버 동작은 임시 PostgreSQL에서 검증했고, 운영 마이그레이션은 트랜잭션으로 적용했다. 빌드 11의 온라인 통합 검증과 TestFlight 배포 결과는 릴리스 기록에 남긴다.
