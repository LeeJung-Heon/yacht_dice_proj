...
닌텐도 『세계의 게임 대전 51』의 Yacht Dice를 레퍼런스로 삼은 iOS 네이티브 야추 다이스 게임에서 출발해 지금은 시작 화면이 요트 다이스·오목·컵퐁·알까기 네 게임의 허브가 되었고 오목·컵퐁까지 실제로 눌리며(알까기는 타일만 있는 준비 중), RealityKit 3D 주사위와 이벤트 소싱 규칙 엔진 위에 세로 화면 전용으로 만들었으며 요트는 혼자 연습·컴퓨터 대전(3단계)·같은 기기 2~4인을, 오목·컵퐁은 같은 기기 2인을 지원하고 세 게임 모두 Supabase 방 코드 온라인 대전과 선택적인 Game Center 로그인·전적·리더보드를 지원한다.
